...
import {ICorkOracleFactory, CorkCorkOracleType, PriceFeedParams} from "/../interfaces/ICorkOracleFactory.sol";
...
contract CorkOracleFactory is OwnableUpgradeable, UUPSUpgradeable, ICorkOracleFactory {
    ...
    /// @inheritdoc ICorkOracleFactory
    function createLinearDiscountOracle(
        address ct,
        uint256 baseDiscountPerYear
    ) external returns (LinearDiscountOracle oracle) {
    ...
    /// @inheritdoc ICorkOracleFactory
    function createLinearDiscountOracleWithMarket(
        Id marketId,
        uint256 epoch,
        uint256 baseDiscountPerYear
    ) external returns (LinearDiscountOracle oracle) {
