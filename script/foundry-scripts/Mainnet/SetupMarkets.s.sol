...
//     CorkConfig public config = CorkConfig(0x6B5CF024365D5d5d0786673780CA7E3F07f85B63);
//     ModuleCore public moduleCore = ModuleCore(0xe3EF345391654121f385679613Cea79A692C2Dd8);
...
//         configureFees(
//             USDe, sUSDS, sUSDS_USDe_Expiry, sUSDS_USDe_RedemptionFee, sUSDS_USDe_RepurchaseFee, sUSDS_USDe_AmmBaseFee
//         );

//         setupMarket(USDT, sUSDe, sUSDe_USDT_Expiry, sUSDe_USDT_ARP, sUSDe_USDT_ExchangeRate);
//         configureFees(
//             USDT, sUSDe, sUSDe_USDT_Expiry, sUSDe_USDT_RedemptionFee, sUSDe_USDT_RepurchaseFee, sUSDe_USDT_AmmBaseFee
//         );
//         console.log("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
//         vm.stopBroadcast();
//     }
...
//         config.initializeModuleCore(paToken, raToken, initialARP, expiryPeriod);
//         console.log("New Market created");

//         Id id = moduleCore.getId(paToken, raToken, expiryPeriod);
//         config.issueNewDs(id, exchangeRate, 0, block.timestamp + 7200, block.timestamp + 30 minutes);
